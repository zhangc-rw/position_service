#ifndef _NBIOT_DEVICE_H_
#define _NBIOT_DEVICE_H_
...
#include <string.h>
#include <pthread.h>
#define CONFIG_FILE_PATH "./conf/config.cfg"
#define PROCESS_FILE_PATH "./conf/process.cfg"
...
typedef struct _command_arr
{
	command commd[COMMAND_MAX];
...
typedef struct _nbiot_message{
	...
	uint32_t  createl_time;/*4byte 消息生成时间可以使用从运营商获得的时间，表示当前消息的生成时间。其值为长整型，代表从2018年1月1日零时起的秒数。*/
